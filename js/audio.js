...
const Audio = {
  ...
  speak(text, lang = 'en-US') {
    ...
    // Hủy bỏ các câu nói trước đó đang bị kẹt (rất hay gặp trên mobile)
    window.speechSynthesis.cancel();
    
    // Fix lỗi iPhone đọc chữ cái thành "Capital A"
    ...
    // Cố gắng tìm giọng tiếng Anh (US hoặc bất kỳ giọng EN nào)
    let voices = this.voices.length > 0 ? this.voices : window.speechSynthesis.getVoices();
    if (voices.length > 0) {
      // Ưu tiên giọng US (Google US, iOS Samantha, v.v.), nếu không thì giọng EN bất kỳ
